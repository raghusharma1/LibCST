...
class Test_FunWithFuncDefsFoo105:
    # Test Scenario 1
    def test_yield_sequence(self):
        # Arrange
        bar = [i for i in range(1, 11)]  # a list of distinct numbers
        gen = foo(bar)
        
        # Act & Assert
        for num in bar:
            assert next(gen) == num  # check if generator is yielding correct sequence
    ...
    def test_pause_resume_generator(self):
        ...
        assert num == 3  # pause generator at 3rd number

        # Act part 2 & Assert
        for num in range(4, 11):
            assert next(gen) == num  # check if generator resumes correctly
    ...
    def test_deplete_generator(self):
        ...
        # Act & Assert
        for _ in range(10):
            next(gen)  # deplete generator

        with pytest.raises(StopIteration):  # check if StopIteration is rasied after depletion
            next(gen)
