...
# ********RoostGPT********
import pytest
from _metadata_dependent import MetadataDependent, LazyValue
from libcst._nodes.base import CSTNode
from libcst.metadata.base_provider import BaseMetadataProvider

class Test_MetaDataDependentGetMetadata342:
    ...
    def test_retrieval_of_metadata(self):
        instance = MetadataDependent()
        instance.METADATA_DEPENDENCIES = {self.MockMetadataProvider}
        instance.metadata = {self.MockMetadataProvider: {self.DummyNode(): 'test_value'}}
        
        result = instance.get_metadata(self.MockMetadataProvider, self.DummyNode())
        
        assert result == 'test_value'

    def test_undeclared_metadata_dependencies_exception(self):
        instance = MetadataDependent()
        instance.METADATA_DEPENDENCIES = set()
        
        with pytest.raises(KeyError, match="is not declared as a dependency in MetadataDependent.METADATA_DEPENDENCIES."):
            instance.get_metadata(self.MockMetadataProvider, self.DummyNode())

    def test_unset_metadata_exception(self):
        instance = MetadataDependent()
        instance.METADATA_DEPENDENCIES = {self.MockMetadataProvider}
        
        with pytest.raises(KeyError, match="is a dependency, but not set; did you forget a MetadataWrapper?"):
            instance.get_metadata(self.MockMetadataProvider, self.DummyNode())
    ...
    def test_get_metadata_with_default_value(self):
        ...
        result = instance.get_metadata(self.MockMetadataProvider,self.DummyNode(), "default_value")
    ...
    def test_lazyvalue_metadata_retrieval(self):
        instance = MetadataDependent()
        instance.METADATA_DEPENDENCIES = {self.MockMetadataProvider}
        instance.metadata = {self.MockMetadataProvider: {self.DummyNode(): LazyValue(lambda: 'lazy_result')}}
